...
if (mediaItems.length > 0) {
  ...
  // fallback لو مفيش media
  const galleryItem = document.createElement('div');
  galleryItem.className = 'gallery-item';
  galleryItem.innerHTML = `
    <img src="${resolveAsset(project.cover_image)}" alt="${project.name} Cover" loading="lazy" />
    <div class="gallery-overlay">
...
function toMediaObject(m) {
  if (!m) return null;

  // لو سترينج زي "assets/covers/1.png"
  if (typeof m === 'string') {
    const isGif = /\.gif(\?.*)?$/i.test(m);
    return { type: isGif ? 'gif' : 'screenshot', url: resolveAsset(m) };
  }

  // لو أوبجكت وبيستخدم url/src
  if (typeof m === 'object') {
    const raw = m.url || m.src || '';
    const isGif = /\.gif(\?.*)?$/i.test(raw);
    return {
      type: m.type || (isGif ? 'gif' : 'screenshot'),
      url: resolveAsset(raw)
    };
  }
